...
import type { SteelDealNote } from '@/lib/supabase/types'
...
interface SteelDealNotesProps {
  dealId: string
  notes: SteelDealNote[]
}

export default function SteelDealNotes({ dealId, notes }: SteelDealNotesProps) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setError(null)
    try {
      // Author is stamped server-side from the signed-in viewer.
      const res = await fetch(`/api/steel/deals/${dealId}/notes`, {
        method: 'POST',
    ...
    <div className="space-y-4">
      ...
      <form onSubmit={handleSubmit} className="space-y-2">
        <textarea
          ...
          placeholder="Log a call, a site visit, a quote revision, or a next step…"
