...
function SectionHeader({ children }: { children: React.ReactNode }) {
  return (
    <p className="label-caps text-muted-foreground mt-6 mb-3 border-t border-border pt-5">
      {children}
    </p>
  )
}
          ...
          <div className="grid grid-cols-[1fr_120px_80px_120px_32px] gap-0 bg-muted border-b border-border">
            {(['Milestone', 'Amount', 'Drawn', 'Target Date', ''] as const).map(
              (h, i) => (
                <div
                  key={i}
                  className="px-2 py-1.5 label-caps text-muted-foreground border-r last:border-r-0 border-border"
                >
                  {h}
                </div>
              )
  ...
  if (mode === 'form') {
    return (
      <div className="max-w-2xl space-y-6">
        ...
        {/* Capital stack */}
        <div className="space-y-4">
          <p className="label-caps text-muted-foreground border-t border-border pt-5">
            Capital Stack
          </p>
          <div className="grid grid-cols-2 gap-4">
            <CurrencyField
          ...
          <div className="rounded-lg border border-border overflow-hidden">
            ...
            {drawSchedule.map((entry, idx) => (
              ...
              >
                ...
                <div className="px-3 py-2.5 text-sm font-medium text-foreground border-r border-border tnum">
