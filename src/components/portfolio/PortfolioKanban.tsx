    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap items-center gap-3">
        <select
          ...
          className="text-sm border border-slate-200 dark:border-border rounded-md px-3 py-1.5 bg-white dark:bg-card text-slate-700 dark:text-slate-200"
        >
        ...
        <div className="ml-auto flex gap-1">
          <button
            ...
            className={`px-3 py-1.5 text-xs font-medium rounded-md border ${viewMode === 'kanban' ? 'bg-slate-900 dark:bg-white/15 text-white border-slate-900 dark:border-white/20' : 'bg-white dark:bg-card text-slate-600 dark:text-slate-300 border-slate-200 dark:border-border'}`}
          >
            Kanban
          </button>
          <button
            onClick={() => setViewMode('list')}
            className={`px-3 py-1.5 text-xs font-medium rounded-md border ${viewMode === 'list' ? 'bg-slate-900 dark:bg-white/15 text-white border-slate-900 dark:border-white/20' : 'bg-white dark:bg-card text-slate-600 dark:text-slate-300 border-slate-200 dark:border-border'}`}
          >
            List
          </button>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {COLUMNS.map(status => (
          <div key={status} className="bg-white dark:bg-card rounded-lg border border-slate-200 dark:border-border px-4 py-3">
            <div className="text-xs text-slate-500 dark:text-muted-foreground font-medium uppercase tracking-wider">{SITE_STATUS_LABELS[status]}</div>
            <div className="text-2xl font-bold text-slate-900 dark:text-foreground mt-1">{grouped[status]?.length ?? 0}</div>
          </div>
        ))}
      </div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-slate-100 dark:border-border/60 bg-slate-50 dark:bg-muted/50">
                <th className="text-left px-4 py-2.5 font-medium text-slate-500 dark:text-muted-foreground">#</th>
                <th className="text-left px-4 py-2.5 font-medium text-slate-500 dark:text-muted-foreground">Site</th>
                <th className="text-left px-4 py-2.5 font-medium text-slate-500 dark:text-muted-foreground">Location</th>
                <th className="text-left px-4 py-2.5 font-medium text-slate-500 dark:text-muted-foreground">Corridor</th>
                <th className="text-left px-4 py-2.5 font-medium text-slate-500 dark:text-muted-foreground">Status</th>
                <th className="text-left px-4 py-2.5 font-medium text-slate-500 dark:text-muted-foreground">Role</th>
                <th className="text-right px-4 py-2.5 font-medium text-slate-500 dark:text-muted-foreground">Components</th>
                <th className="text-right px-4 py-2.5 font-medium text-slate-500 dark:text-muted-foreground">Capital (Mid)</th>
              </tr>
            </thead>
            <tbody>
              {filtered.map(site => (
                <tr key={site.id} className="border-b border-slate-50 dark:border-border/40 hover:bg-slate-50 dark:hover:bg-muted/50">
                  <td className="px-4 py-2.5 text-slate-400 dark:text-muted-foreground font-mono text-xs">{site.site_number ?? '—'}</td>
                  <td className="px-4 py-2.5">
                    <Link href={`/portfolio/sites/${site.id}`} className="font-medium text-slate-900 dark:text-foreground hover:text-blue-600">
                      {site.name}
                    </Link>
                  </td>
                  <td className="px-4 py-2.5 text-slate-600 dark:text-slate-300">{[site.city, site.state].filter(Boolean).join(', ') || '—'}</td>
                  <td className="px-4 py-2.5 text-slate-500 dark:text-muted-foreground text-xs">{site.corridor_name ?? '—'}</td>
                  <td className="px-4 py-2.5">
...
function SiteCard({ site }: { site: SiteSummary }) {
  return (
    <Link
      ...
      className="block bg-white dark:bg-card rounded-lg border border-slate-200 dark:border-border p-3.5 hover:border-slate-300 dark:hover:border-border hover:shadow-sm transition-all"
    >
      <div className="flex items-start justify-between gap-2">
        <div className="min-w-0">
          ...
          <h3 className="text-sm font-semibold text-slate-900 dark:text-foreground truncate leading-tight mt-0.5">{site.name}</h3>
          <p className="text-xs text-slate-500 dark:text-muted-foreground mt-0.5">{[site.city, site.state].filter(Boolean).join(', ')}</p>
        </div>
        {site.is_lead_site && (
      ...
      <div className="mt-2.5 flex items-center justify-between text-xs text-slate-500 dark:text-muted-foreground">
