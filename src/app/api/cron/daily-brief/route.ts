...
export async function GET(request: NextRequest) {
  ...
  const [
    ...
    supabase
      ...
      .in('bucket', ['now', 'soon'])
      .order('sort_order'),
    // Capital raise — all fail quietly (null) pre-migration
    supabase
      .from('investors')
  ...
  // Capital raise summary — per-raise standing, totals, and the follow-ups
  // that need a human this week
  ...
  if ((investors ?? []).length > 0) {
    ...
    // Where each open raise stands (tranches fill sequentially from committed money)
    for (const raise of raiseRows ?? []) {
      ...
      const nextTranche = fills.find((t) => t.committed < t.amount)
      investorLines.push(
        `- Raise "${raise.name}" (${raise.status}): target ${raise.target_amount != null ? money(raise.target_amount) : 'unset'} — ${money(levels.committed)} committed / ${money(levels.funded)} funded / ${money(levels.potential)} potential if all indications convert${nextTranche ? `; filling ${nextTranche.label} (${money(nextTranche.committed)} of ${money(nextTranche.amount)})` : fills.length > 0 ? '; all tranches committed' : ''}`
      )
    }
    investorLines.push(
