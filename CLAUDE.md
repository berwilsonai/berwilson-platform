...
- **Remaining from the agreed map idea list (punch through next):** marker overlap/clustering at low zoom (urgent once all ~97 are placed), stage/status filter + pipeline-vs-active marker distinction, value-scaled markers, deep links (`/map?project=`) + "View on map" on project pages + ⌘K, on-map project search, present-mode animated rail dashes, photo lightbox in the sheet. Deliberately skipped: 3D terrain, heatmaps, time sliders.
