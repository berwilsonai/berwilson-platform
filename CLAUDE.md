...
- **Safety rules in `getViewer` (mirrored in middleware):** pre-migration (columns missing) → everyone admin (today's behavior, code ships first); bootstrap (migration applied but nobody linked) → everyone admin (can't lock yourself out); linked → their role; unlinked after linking has started → member. Last-active-admin demotion blocked server-side.
...
> **DB push status (2026-07-03, user access):** `20260704000005_user_access.sql` to apply, then `npm run gen-types`. **Pre-migration: zero behavior change** (every check degrades to admin). Post-migration bootstrap: Richard opens `/settings/users` → invites himself with his existing login email (links, stays admin) → same for Eric → then invite/scope everyone else. Everyone stays admin until the FIRST account is linked.
