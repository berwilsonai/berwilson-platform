...
- **Fully offline basemap — zero external requests at view time.** MapLibre GL JS + Protomaps `.pmtiles` served by new `GET /api/map/tiles` (fs byte-range streaming; 206/416 semantics unit-tested incl. pmtiles magic bytes; missing archive → 503 with a setup hint, surfaced in the map UI). Archive at `~/berwilson-data/maps/us.pmtiles` **outside the app dir** (survives the deploy rsync `--delete`); override `MAP_PMTILES_PATH`. Style built in code (`src/lib/map/style.ts`, `@protomaps/basemaps` layers, light/dark flavors — follows the app's dark-mode toggle live via a MutationObserver on `<html>`); fonts/sprites vendored to `public/basemaps/` (gitignored; the deploy rsync ships the working tree). One-time setup `scripts/setup-map-data.sh` (pmtiles CLI CONUS extract, ~17-20GB); `deploy/deploy-to-studio.sh` gained an idempotent map-data rsync block; documented in `deploy/README.md`. New deps: `maplibre-gl`, `pmtiles`, `@protomaps/basemaps` (rendering libs only — §11 intact); maplibre is SSR-firewalled behind `next/dynamic ssr:false` and stays in the /map chunk.
...
- **Verified end-to-end on the real stack:** tile route 13/13 range-semantics checks; magic-link login → `/map` 200 authed / 307 unauthed; tiles 206 through the full middleware stack; PATCH place/revert, validation rejects (bad lat/icon/geometry), and LineString round-trip confirmed against the Studio DB. `tsc` + eslint + full `next build` clean.
