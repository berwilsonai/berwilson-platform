...
- **Not yet live-tested:** the Mac Studio was offline on the tailnet all session (last seen 3d). Richard's checklist to go live: wake the Studio + keep it awake (Energy settings/caffeinate), Tailscale running, LM Studio → Developer → enable server on the network (port 1234), load Qwen3-30B-A3B (Instruct/2507 recommended over the thinking variant) + an embedding model, then uncomment the env block in `.env.local` (with `EMBEDDINGS_PROVIDER=gemini`) and `npm run dev`. First live test: `/intel` agent chat, a manual-paste extraction, a PDF upload.
